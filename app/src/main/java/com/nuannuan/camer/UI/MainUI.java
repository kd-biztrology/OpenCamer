...
public class MainUI {
  ...
  public void setTakePhotoIcon() {
    ...
    if (main_activity.getPreview() != null) {
      ...
      view.setContentDescription(
          main_activity.getResources().getString(switch_video_content_description));
    }
  }
      ...
      public void run() {
        ...
        if (!main_activity.getPreview().isVideo() || !main_activity.getPreview().supportsFlash()) {
          popupButton.setVisibility(
              visibility); // still allow popup in order to change flash mode when recording video
        }
      }
    });
  }
  ...
  public void toggleExposureUI() {
    ...
    SeekBar exposure_time_seek_bar =
        ((SeekBar) main_activity.findViewById(R.id.exposure_time_seekbar));
    int exposure_time_visibility = iso_seek_bar.getVisibility();
    boolean is_open = exposure_visibility == View.VISIBLE
        || iso_visibility == View.VISIBLE
        || exposure_time_visibility == View.VISIBLE;
    if (is_open) {
      clearSeekBar();
      ...
      if (main_activity.getPreview().usingCamera2API() && !value.equals(
          main_activity.getPreview().getCameraController().getDefaultISO())) {
        ...
        if (main_activity.getPreview().supportsExposures()) {
          ...
          ZoomControls seek_bar_zoom =
              (ZoomControls) main_activity.findViewById(R.id.exposure_seekbar_zoom);
