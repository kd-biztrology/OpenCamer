...
public abstract class CameraController {
  ...
  public abstract void autoFocus(final AutoFocusCallback cb);
  ...
  protected SupportedValues checkModeIsSupported(List<String> values,String value,
      String default_value) {
    if (values != null
        && values.size()
        > 1) { // n.b., if there is only 1 supported value, we also return null, as no point offering the choice to the user (there are some devices, e.g., Samsung, that only have a scene mode of "auto")

      for (int i = 0; i < values.size(); i++) {
        Logger.d(TAG,"supported value: " + values.get(i));
      }
      // make sure result is valid
      if (!values.contains(value)) {

        Logger.d(TAG,"value not valid!");
        ...
        Logger.d(TAG,"value is now: " + value);
      }
